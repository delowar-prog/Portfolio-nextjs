...
const Banner = () => {
  const position = ['Backend Developer', 'Laravel Developer', 'Fullstack Developer']
  return (
    <div id='home' className='max-w-contentContainer py-8 md:py-24 mx-auto px-8 md:px-28'>
      <div className='w-full md:w-[80%] my-5'>
        <motion.h4 initial={{y:-2, opacity:0}} animate={{y:0, opacity:1}} transition={{duration:0.1, delay:0.1}} className='text-lg text-textGreen'>Hi, Me </motion.h4>
        ...
        <motion.p initial={{y:-2, opacity:0}} animate={{y:0, opacity:1}} transition={{duration:0.4, delay:0.4}} className='text-textLight text-md mb-3'>A dedicated Laravel developer with a passion for creating dynamic and user friendly web application. I have completed few projects from design to deployment using PHP, Laravel, Livewire, React, Bootstrap, Tailwind & Mysql with 3 years Job experience.</motion.p>
        <motion.a initial={{y:-2, opacity:0}} animate={{y:0, opacity:1}} transition={{duration:0.5, delay:0.5}} href='#about'>
